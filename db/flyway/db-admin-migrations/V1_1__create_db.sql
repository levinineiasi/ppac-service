...
(
    ...
    PERIOD_COUNT                  INTEGER NOT NULL CHECK (PERIOD_COUNT >= 0 AND PERIOD_COUNT <=24),
    PERIOD_TYPE                   VARCHAR(12) CHECK (PERIOD_TYPE IN ('WEEKS', 'MONTHS')),
    OPEN_POSITIONS                INTEGER NOT NULL CHECK (OPEN_POSITIONS >= 0 AND OPEN_POSITIONS <=30),
    ACCEPT_ON_CLOSING_OPPORTUNITY BOOLEAN NOT NULL,
    SIGN_AGREEMENT                BOOLEAN NOT NULL,
    AVAILABLE                     BOOLEAN NOT NULL,
    TITLE                         VARCHAR(30) CHECK (LENGTH(TITLE) >= 5),
    DESCRIPTION                   VARCHAR(1000) CHECK (LENGTH(DESCRIPTION) >= 40),
